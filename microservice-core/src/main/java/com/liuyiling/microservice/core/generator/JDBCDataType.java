...
/**
 * 对于DB操作的数据类型有三种：
 * DB数据类型 -- JDBC数据类型（java.sql.Types） -- Java数据类型
 * 该类为 JDBC数据类型（java.sql.Types）对应的值
 * 举例：在SQL DB数据类型中为BIT，则对应到JDBC数据类型为BIT(-7)
 * 代码从java.sql.JDBCType复制
 * @author liuyiling
 */
public class JDBCDataType {
    ...
    public static String getJdbcType(int code) {
        Field[] fields = JDBCDataType.class.getDeclaredFields();
